...
from HDR_hotspot import bam2hotspot_multi, pileup2hotspot, bam2hotspot
...
def HDR_master(mut_file, bam_file, chrom, threads, HDR_config, pileup_file=''):
    ...
    HDR_dfs = []
    for chrom in chroms:
        hdr_df = HDR_run(mut_df, mut_file, bam_file, chrom, threads, HDR_config, pileup_file)
...
def HDR_run(mut_df, mut_file, bam_file, chrom, threads, HDR_config, pileup_file):
    ...
    if pileup_file:
        ...
        hotspot_df = bam2hotspot_multi(bam_file, chrom=chrom, HDR_config=HDR_config, mut_df=mut_df, threads=threads)
        show_output(
