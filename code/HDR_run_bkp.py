...
def HDR_run(mut_file, bam_file, chrom, threads, HDR_config, pileup_file=''):
    ...
    show_output(
        f"Starting HDR analysis of {mut_file}{cores}. [MIN_SIM={HDR_config['MINSIM']}, PAD={HDR_config['PAD']}]")
...
def HDR_run(mut_file, bam_file, chrom, threads, HDR_config, pileup_file=''):
    ...
    if pileup_file:
        hotspot_df = pileup2hotspot(mut_df, pileup_file, chrom=chrom, HDR_config=HDR_config)
        show_output(
            f"Detected {len(hotspot_df.index)} putative HDR lanes in {pileup_file}.")       
    else:
        hotspot_df = bam2hotspot_multi(mut_df, bam_file, chrom=chrom, threads=threads, HDR_config=HDR_config)
        show_output(
